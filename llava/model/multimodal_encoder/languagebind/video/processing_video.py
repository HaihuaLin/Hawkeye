...
def load_and_transform_video(
    ...
    if '.jpg' in video_path:
        image = load_and_transform_image(video_path, get_image_transform())
        return image.unsqueeze(1).expand(-1, 8, -1, -1)
    else:
        if video_decode_backend == 'pytorchvideo':
            ...
            # frame_id_list = np.linspace(0, duration-1, 40, dtype=int)
            # frame_id_list = np.arange(0, duration-1, 3000, dtype=int)
...
class LanguageBindVideoProcessor(ProcessorMixin):
    ...
    def __call__(self, images=None, text=None, context_length=77, return_tensors=None, **kwargs):
        ...
        if images is not None:
            ...
            # image_features = torch.split(image_features[0], 1, dim=1)
